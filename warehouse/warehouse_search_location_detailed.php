...
<p>
<center><a href="http://www.nutritionwarehouse.com.au/warehouse/warehouse_search.php"><img src="http://www.nutritionwarehouse.com.au/warehouse/barcode.png" style="width:50px;"></a>&nbsp;&nbsp;&nbsp;&nbsp;<img src="http://www.nutritionwarehouse.com.au/warehouse/location.png" style="width:50px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://www.nutritionwarehouse.com.au/warehouse/warehouse_remap.php"><img src="http://www.nutritionwarehouse.com.au/warehouse/remap.png" style="width:50px;"></a><br>&nbsp;<br>
<form action="" method="post" name="boogie">  
</center>
&nbsp;&nbsp;Location:
<center>
<input type="text" placeholder="Location" name="term" style="width:200px; height:20px;"/>
...
$sql = "SELECT * FROM xcart_products, xcart_variants WHERE xcart_products.productid = xcart_variants.productid and xcart_variants.rexlocation = '$term'"; 
$r_query = mysql_query($sql);

while ($row = mysql_fetch_array($r_query)){  
echo '<br /> Warehouse Location: <b>' .$row['rexlocation'];  
echo '</b><br>Product REX Code: ' .$row['productcode'];  
echo '</b><br>Product Barcode: ' .$row['SKU'];  
echo '<br /> Available Stock: ' .$row['avail'];  
echo '<br />';
echo '<br />' .$row['product']. '<br />';
...
$sql = "SELECT xcart_product_options_lng.option_name FROM xcart_products, xcart_variants, xcart_variant_items, xcart_product_options_lng WHERE xcart_products.productid = xcart_variants.productid and xcart_variants.variantid = xcart_variant_items.variantid and xcart_variant_items.optionid = xcart_product_options_lng.optionid and xcart_variants.rexlocation = '$term'";
$r_query = mysql_query($sql); 
...
$sql = "SELECT SUM(Amount) As Website_Sales_30days, ROUND(AVG(price),2) As Average_Price FROM `xcart_order_details`, `xcart_orders`, `xcart_variants` Where `xcart_orders`.`orderid` = `xcart_order_details`.`orderid` and `xcart_order_details`.`productcode` = `xcart_variants`.`productcode` and `xcart_orders`.`status` <> 'F' and `xcart_orders`.`status` <> 'D' and `xcart_orders`.`date` > UNIX_TIMESTAMP(DATE_ADD(CURDATE(),INTERVAL -30 DAY)) and `xcart_variants`.`rexlocation` = '$term'";
$r_query = mysql_query($sql); 

while ($row = mysql_fetch_array($r_query)){  
echo '<br /> Last 30 Days:&nbsp;&nbsp; '.$row['Website_Sales_30days'];  
...
$sql = "SELECT SUM(Amount) As Website_Sales_30days, ROUND(AVG(price),2) As Average_Price FROM `xcart_order_details`, `xcart_orders`, `xcart_variants` Where `xcart_orders`.`orderid` = `xcart_order_details`.`orderid` and `xcart_order_details`.`productcode` = `xcart_variants`.`productcode` and `xcart_orders`.`status` <> 'F' and `xcart_orders`.`status` <> 'D' and `xcart_orders`.`date` > UNIX_TIMESTAMP(DATE_ADD(CURDATE(),INTERVAL -90 DAY)) and `xcart_variants`.`rexlocation` = '$term'";
$r_query = mysql_query($sql); 

while ($row = mysql_fetch_array($r_query)){  
echo '<br /> Last 90 Days:&nbsp;&nbsp; '.$row['Website_Sales_30days'];
...
<div style="position:absolute; bottom: 0; width:100%; height: 30px; text-align:center; vertical-align: middle;"><a href="http://www.nutritionwarehouse.com.au/warehouse/warehouse_search_location.php">Simple Search</a></div>
