...
var AW_AjaxCartPro = {
    ...
    fire: function(url, parameters, observer) {
        ...
        this.connector.sendRequest(url, parameters,
            function(response) {
                if(response.removed_freeshpping ==1 ){
...
var AW_AjaxCartProUI = {
    ...
    _show: function(el) {
        ...
        jQuery("body").addClass("show_quickview");
    ...
    _hide: function(el) {
        jQuery("body").removeClass("show_quickview");
    ...
    _resizeBlock: function(el) {
        ...
        var docHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
        var xy = this._collectPos(el, 'center', this.config.dialogsVAlign);
        // var _left =  (docWidth  - el.getWidth()) / 2;
        if (xy[0] < 15) {
            xy[0] = 15;
            el.setStyle({
