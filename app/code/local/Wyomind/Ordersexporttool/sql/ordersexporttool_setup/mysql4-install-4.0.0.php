...
CREATE TABLE IF NOT EXISTS `' . $this->getTable('ordersexporttool_profiles') . '` (
  ...
  `file_mail_enabled` INT(1) DEFAULT \'0\',
  `file_mail_subject` VARCHAR(200),
  `file_mail_recipients` VARCHAR(300),
  `file_mail_message` TEXT,
  `file_mail_zip` INT(1) default \'0\',
  `file_mail_one_report` INT(1) default \'0\',
  `file_local_enabled` INT(1) DEFAULT \'1\',
  `file_product_filter` INT(1) DEFAULT \'0\',
  PRIMARY KEY  (`file_id`)
...
if (version_compare(Mage::getVersion(), '1.4.0', '<')) {
    $installer->getConnection()->addColumn(
            $installer->getTable('sales_order'), 'export_flag', 'text;'
    );
    ...
    $installer->getConnection()->addColumn(
            $installer->getTable('sales_flat_order'), 'export_flag', 'text;'
    );
    $installer->getConnection()->addColumn(
            $installer->getTable('sales/order_grid'), 'export_flag', 'text;'
    );
    $installer->getConnection()->addColumn(
            $installer->getTable('sales/quote_item'), 'export_to', 'int(3) NULL;'
    );
    $installer->getConnection()->addColumn(
            $installer->getTable('sales/order_item'), 'export_to', 'int(3) NULL;'
    );
}
