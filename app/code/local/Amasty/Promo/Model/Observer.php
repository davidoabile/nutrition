        ...
        foreach ($observer->getQuote()->getAllItems() as $item){
            ...
            if(!empty($samebonus) && in_array($samebonus,$ruleid)){
               if($item->getParentItemId()){
                ...
                $buy1bonus[$item->getSku()] = $item->getQty();
            }
        }
        /*end code*/
        foreach ($observer->getQuote()->getAllItems() as $item)
        {
            if ($item->getIsFree())
            {
                ...
                if (isset($allowedItems['_groups'][$item->getRuleId()])) // Add one of
                {
                    ...
                    /*code for buy x get x bonus*/
                    $_rule_maxqty = Mage::getModel('salesrule/rule')->load($item->getRuleId())->getDiscountQty();
                    if($item->getRuleId() != Mage::getStoreConfig('ampromo/gift/ruleid')){ /*ignore freegift rule*/
                        $buyxlist=Mage::helper('ampromo/bonus')->getBuyXBonusItem($item->getRuleId());
                        if(!empty($buyxlist)){
                            if(!empty($_rule_maxqty))$buyxlist= min($buyxlist,$_rule_maxqty); 
                            $item->setQty($buyxlist);
                        }
                    }
                }
