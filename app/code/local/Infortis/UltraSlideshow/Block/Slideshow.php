...
class Infortis_UltraSlideshow_Block_Slideshow extends Mage_Core_Block_Template {
    ...
    public function getParamStaticBlockIds() {
        if (count($this->_blockIds) === 0) {
            $slides = $this->getSlides(); //param: slides
            if ($slides === NULL) { //Param not set
                return array();
            }

            $this->_blockIds = explode(",", str_replace(" ", "", $slides));
        }
    ...
    public function getConfigStaticBlockIds() {
        if (count($this->_blockIds) === 0) {
            ...
            $this->_blockIds = explode(",", str_replace(" ", "", $blockIdsString));
        }
        return $this->_blockIds;
    }
    ...
    public function getBannersHtml() {
        ...
        $bid = trim($bid);
        if ($bid) {
          $content = Mage::app()->getLayout()->createBlock('zblocks/block')->setPosition($bid)->toHtml();
        }
        return $content;
    }
