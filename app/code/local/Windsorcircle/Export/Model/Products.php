			...
			foreach($product->getStoreIds() as $key => $_store){
				...
				if(empty($productData_Adv[$product->getId()])){
					$productData_Adv[$product->getId(). ':' . $_store] = array($product->getId(),
																	...
																	$product->getSku(),
                                                                    (!empty($parentId) ? $parentId : (!empty($parentIdRelation) ? $parentIdRelation : '')),
																	$categoryList,
                    ...
                    foreach ($customAttributes as $customAttribute) {
                        if ($product->getData($customAttribute) === null) {
                            array_push($productData_Adv[$product->getId(). ':' . $_store], '');
                            continue;
                        }
                        $data = $product->getResource()->getAttribute($customAttribute)->getFrontEnd()->getValue($product);
                        array_push($productData_Adv[$product->getId(). ':' . $_store], $helper->formatString($data));
                    }
				}
