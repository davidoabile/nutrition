...
class Moogento_Core_Adminhtml_System_Config_CountrytemplateController extends Mage_Adminhtml_Controller_Action
{
...
{{if vat_id}}VAT: {{var vat_id}}<br />{{/if}}'
    ...
    public function destroyCountrytemplateAction()
    {
        ...
        $countryTemplate = Mage::getModel("moogento_core/countrytemplate")->load($countryTemplateId);
    ...
    public function addDefaultsAction()
    {
        foreach ($this->_templateDefaults as $country => $template) {
            $templateModel = Mage::getModel("moogento_core/countrytemplate");
