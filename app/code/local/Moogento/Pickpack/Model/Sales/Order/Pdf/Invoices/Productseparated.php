     ...
     public function updateData($order_arrs,$product_filter = array())
    {
        ...
        foreach ($this->_orderCollection as $order) {
            ...
            if($this->_printing_format['split_bundle_product'] == 1)
                $this->_itemsCollection[$_orderId] = $order->getAllItems();
            else
                $this->_itemsCollection[$_orderId] = $order->getAllVisibleItems();
            $total_item_count = 0;
            $product_list_order = '';
...
//         Mage::log(memory_get_usage(),'null','moogento_pickpack.log');
        ...
        $this->_printing_format['font_size_overall']    = 15;
        $this->_printing_format['font_size_productline'] = 9;
...
//        $this->printLogo($page,$store_id,$show_top_logo_yn);
        ...
        foreach($this->product_list_arr as $key => $product_id)
        {
            ...
            if (($this->y < 60))
            {
                ...
                if ($this->pagecount == 1) {
                    $this->_setFont($page, $this->_printing_format['font_style_body'], $this->_printing_format['font_size_body'], $this->_printing_format['font_family_body'], $this->_printing_format['non_standard_characters'], $this->_printing_format['font_color_body']);
                    // $this->_setFont($page,$this->_printing_format['font_style_subtitles'], ($this->_printing_format['$font_size_subtitles'] - 2), $this->_printing_format['font_family_subtitles'], $this->_printing_format['non_standard_characters'], $this->_printing_format['font_color_subtitles']);
            ...
            $grey_box_y1 = ($this->y - ($this->_printing_format['font_size_body'] * 1.1 + ($num_lines - 1) * ($this->_printing_format['font_size_body'] + 2)));
            $grey_box_y2 = ($this->y + ($this->_printing_format['font_size_body'] * 0.85));
                    ...
                    if($v['code'] == 'name'){
                        ...
                        if(strlen($print_data) > $max_chars_print){
                            ...
                            while ($token != false) {
                                ...
                                $this->y -= ($this->_printing_format['font_size_body'] + 2);
                                $token = strtok("\n");
                            }
                            $after_print_product_name = $this->y + $this->_printing_format['font_size_body'] + 2;
            ...
            if(isset($this->order_list_of_product_per_option[$product_id]))
            {
                ...
                { 
                    ...
                    $this->y -=$this->_printing_format['font_size_body']*1.25;
                    foreach ($opt_arr  as $opt_each){
                        $opt_each = trim($opt_each, "]");
                        $opt_each = trim($opt_each, "[");
                        $page->drawText($opt_each , 35, $this->y, 'UTF-8');
                        $this->y -= $this->_printing_format['font_size_body']*1.2;
                    }
                    $page->setFillColor($this->_printing_format['white_bkg_color']);
                    $page->setLineColor($this->_printing_format['grey_bkg_color']);
                    $grey_box_y1 = ($this->y - ($this->_printing_format['font_size_body'] * 1.35));
                    $grey_box_y2 = ($this->y + ($this->_printing_format['font_size_body'] * 0.85));
                    $page->drawLine(20,$grey_box_y1+5,$this->_printing_format['padded_right'], $grey_box_y1+5);
                    $this->y -=5;
                    $this->_setFont($page, $this->_printing_format['font_style_body'], $this->_printing_format['font_size_body']-2, $this->_printing_format['font_family_body'], $this->_printing_format['non_standard_characters'], $this->_printing_format['font_color_body']);
    ...
    protected function printOrder(&$page,$product_id,$orderId,$store_id=0)
    {
            ...
            else
            {
                ...
                if($v['code'] == 'customer_name'){
                    ...
                    if(strlen($print_data) > $max_chars_print){
                        ...
                        $after_print_customer_name = $this->y + $this->_printing_format['font_size_body'] + 2;
        ...
        if(isset($tickbox_width) && ($tickbox_width > ($this->_printing_format['font_size_body']*1.2)))
