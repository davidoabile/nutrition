...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Stock extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    public function getPickStock($orders = array())
    {
        ...
        // $min_qty_in_stock[$sku]_yn_default         = 0;
        //     $min_qty_in_stock[$sku]_default         = 1;
        ...
        $stockcheck_yn = 1; //$this->_getConfig('pickpack_showstock_yn_combined', $min_qty_in_stock[$sku]_yn_default, false,'messages');
        ...
        $nameyn = $name_yn; //$this->_getConfig('pickpack_name_yn_combined', $namenudgeYN_default, false,'messages');
        ...
        if ($split_supplier_yn == 1) {
            foreach ($supplier_master as $supplier => $value) {
                ...
                if ($supplier_skip_order == FALSE) {
                    ...
                    if ($picklogo == 1 && ($csv_or_pdf == 'pdf')) {
                        ...
                        $page->drawText($helper->__('Out-of-stock Pick List'), 325, 810, 'UTF-8');
                        ...
                        $page->drawText($helper->__('Out-of-stock Pick List, Supplier') . ' : ' . $supplier, 31, 810, 'UTF-8');
                        $page->setLineColor($font_color_header_zend);
                        $page->setFillColor($font_color_header_zend);
                        $page->setLineWidth(0.5);
                ...
                if ($showcount_yn == 1) {
                    if ($csv_or_pdf == 'pdf') $page->drawText($helper->__('Total quantity') . ' : ' . $total_quantity, 375, $this->y, 'UTF-8');
            ...
            if ($csv_or_pdf == 'pdf') {
                ...
                if ($picklogo == 1) {
                    ...
                    $this->_setFont($page, $font_style_header, ($font_size_header + 4), $font_family_header, $non_standard_characters, $font_color_header);
                    $page->drawText($helper->__('Out-of-stock Pick List'), 31, 810, 'UTF-8');
                    $page->setLineColor($font_color_header_zend);
                    $page->setFillColor($font_color_header_zend);
                    $page->drawRectangle(27, 803, $padded_right, 804);
                }
            ...
            foreach ($sku_master as $key => $sku) {
                ...
                if (!in_array($sku, $processed_skus) && ($sku_stock[$sku] < $min_qty_in_stock[$sku])) {
                    ...
                    if ($csv_or_pdf == 'pdf') {
                        if ($this->y < 15) $page = $this->newPage();
                        $this->_setFont($page, $font_style_body, $font_size_body, $font_family_body, $non_standard_characters, 'Black');
            ...
            if ($showcount_yn == 1) {
                if ($csv_or_pdf == 'pdf') {
                    $page->drawText($helper->__('Total quantity') . ' : ' . $total_quantity, 375, $this->y, 'UTF-8');
                    $this->y -= 20;
