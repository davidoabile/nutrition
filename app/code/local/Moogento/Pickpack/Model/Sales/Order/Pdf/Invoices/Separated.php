...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Separated extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    function getPickSeparated($orders = array(), $from_shipment = 'order')
    {
        ...
        $font_color_subtitles = $font_color_subtitles_default;//trim($this->_getConfig('font_color_subtitles', $font_color_subtitles_default, false, 'general'));
...
{if country}{country}|{/if}";
        ...
        $product_qty_upsize_yn = $this->_getConfig('product_qty_upsize_yn', 0, false, 'picks');
            ...
            foreach ($supplier_master as $key => $supplier) {
                if ((isset($supplier_login) && ($supplier_login != '') && (strtoupper($supplier) == strtoupper($supplier_login))) || !isset($supplier_login) || $supplier_login == '') {
                    ...
                    $each_order_per_page =  $this->_getConfig('pickpack_newpage_per_order',0, false,'picks', $store_id_arr[$order_id]);
                        ...
                        if($this->y < $font_size_subtitles + 20)
                            $condition_new_page = 1;
                        ...
                        if ($supplier_skip_order == FALSE) {
                            ...
                            foreach ($sku_order_id_qty[$order_id] as $sku => $qty) {
                                ...
                                if ($supplier_item_action != 'hide' && trim($supplier_item_action) != '') {
                                    ...
                                    if($font_family_body == 'traditional_chinese' || $font_family_body == 'simplified_chinese'){
                                        $font_family_body_temp = $font_family_body;
                                        $font_family_body = 'helvetica';
                                    }
                                    $this->_setFont($page, 'bold', ($font_size_body), $font_family_body, $non_standard_characters, $font_color_body);
                                    if ($product_qty_upsize_yn == 1 && $qty_string > 1 && $show_bundle_parent) {
                                        if ($product_qty_red == 1) $this->_setFont($page, 'bold', ($font_size_body + 1), $font_family_body, $non_standard_characters, 'darkRed');
                                    ...
                                    if(isset($font_family_body_temp)){
                                        $font_family_body = $font_family_body_temp;
                                        $this->_setFont($page, $font_style_body, $font_size_body, $font_family_body, $non_standard_characters, $font_color_body);
                                    }
                                    //$page->drawText($qty_string, $qtyX, $this->y, 'UTF-8');
        ...
        else 
        {
            ...
            foreach ($product_build as $order_id => $order_build) {
                ...
                if ($condition_new_page == 1){
                    if ($page_count == 1) {
                        $this->_setFont($page, $font_style_subtitles, ($font_size_subtitles - 2), $font_family_subtitles, $non_standard_characters, $font_color_subtitles);
                        $page->drawText('-- ' . $helper->__('Page') . ' ' . $page_count . ' --', 250, ($font_size_subtitles - 7), 'UTF-8');
                    }
                    $page = $this->newPage();
                    $page_count++;
                    $this->_setFont($page, $font_style_subtitles, ($font_size_subtitles - 2), $font_family_subtitles, $non_standard_characters, $font_color_subtitles);
                ...
                if($giftmessage_yn == 1){
                    if(isset($gift_msg_array[$order_id])){
                        ...
                        if($message_title_tofrom_yn == 1){
                            //$this->_setFont($page, "bold", $font_size_body, $font_family_body, $non_standard_characters, $font_color_body);
                            //$this->_setFont($page, 'bold', ($font_size_message), $font_family_message, $non_standard_characters, $font_color_message, $page);
                            //$this->y = $this->showToFrom($message_title_tofrom_yn, $to_from, $msgX + $font_size_body/3, $this->y, $to_from_from, $font_size_body, $page);
                        ...
                        $width_title = $this->getWidthString("Gift Message: ", $font_size_body);
                        // print the gift message content
                        //$this->_setFont($page, $font_style_message, ($font_size_gift_message - 1), $font_family_message, $non_standard_characters, $font_color_message);
                        $this->_setFont($page, $font_style_body, $font_size_body, $font_family_body, $non_standard_characters, $font_color_body);
                        $this->y = $this->drawOrderGiftMessage($gift_msg_array, $msgX + $font_size_body/3 + $width_title, $font_size_body, $this->y, $page);
...
//                     $dsku = $value['sku'];
                    ...
                    if ($this->y < $font_size_subtitles + 5) {
                        if ($page_count == 1) {
                            $this->_setFont($page, $font_style_subtitles, ($font_size_subtitles - 2), $font_family_subtitles, $non_standard_characters, $font_color_subtitles);
                   ...
                   $page->drawRectangle($tickbox2_X, ($this->y), $tickbox2_X + 7, ($this->y + 7));
                    ...
                    if (($product_images_yn == 1) && isset($sku_image_paths[$sku]['path'][0]) && $show_bundle_parent) {
                        ...
                        try{
                            ...
                            if($giftmessage_yn == 1 && $value['has_message'] == 1)
                                $this->y -= (2*$font_size_body + 20 + $sku_image_paths[$sku]['height']);
                            else
                                $this->y -= (2*$font_size_body + 35 + $sku_image_paths[$sku]['height']);
                    ...
                    if($giftmessage_yn == 1 && $show_bundle_parent){
                        ...
                        if ($value['has_message'] == 1) {
                            ...
                            foreach ($value['message-content'] as $gift_msg_line) {
                                $temp_height += 2 * $font_size_body + 3;
                            }
                            foreach ($value['message-content'] as $gift_msg_line) {
                                if (($this->y) < 40) {
                                    if ($page_count == 1 && isset($return_address_yn) && $return_address_yn == 0 && isset($bottom_shipping_address_yn) && $bottom_shipping_address_yn == 0) {
                    ...
                    if (isset($sku_bundle[$order_id][$sku]) && is_array($sku_bundle[$order_id][$sku])) {
                        ...
                        if($show_bundle_parent)
                        $this->y -= ($font_size_body);
                        $this->_setFont($page, $font_style_body, ($font_size_body - 2), $font_family_body,
                        $non_standard_characters, $font_color_body);
                        if( $is_split && $show_bundle_parent_yn == 1) {
