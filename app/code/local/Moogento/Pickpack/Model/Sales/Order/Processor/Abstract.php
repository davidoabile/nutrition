...
class Moogento_Pickpack_Model_Sales_Order_Processor_Abstract extends Varien_Object
{
  ...
  protected function _saveAsFile($pdf, $fileName, $storeId = 0)
							...
							try{
								...
								switch($this->flagColumn)
								{
									case 'pack_printed':
										...
										break;
									case 'invoice_printed':
										$flagautoaction->setInvoice_printed($flag_value + 1);
										break;
									case 'separate_printed':
    ...
    public function processBulk_pre($digitDay)
    { 
                            ...
                            if (count($orderIds)) {
                                ...
                                if ($this->_getConfigFlag("auto_processing_check", $storeId)) {
                                    foreach ($orderIds as $_orderId) {
                                        ...
                                        if($checkField == 'custom_attribute')
                                            $postfix =  'szy_check_custom_value_1';
                                        else
                                            if($checkField == 'custom_attribute2')
                                                $postfix =  'szy_check_custom_value_2';
                                            else
                                                $postfix =  'szy_check_custom_value_3';
    ...
    public function processBulk_pre2($digitDay)
    { 
        $stores = Mage::app()->getStores(true);
        $ordersByStores = array();
        $avalable_orders_processing = 3;
    ...
    { 
        ...
        foreach ($ordersByStores as $storeId => $storeOrderIds) {
            ...
            foreach($storeOrderIds as $orderId)
            {
                ...
                {
                    try{
                       //  $flagautoaction = Mage::getModel('pickpack/flagautoaction')->load($orderId,'orderid');
                        switch($this->flagColumn)
                        {
                            case 'pack_printed':
