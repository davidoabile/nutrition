...
class AW_Ajaxcartpro_Model_Observer
{
    ...
    public function sendResponseBefore($observer)
    {
        ...
        if ($request->getParam('awacp', false)) {
            ...
            Mage::getSingleton('core/session')->setAwproductrecentlyadd(Mage::app()->getLayout()->createBlock('catalog/product_list')->setTemplate('ajaxcartpro/added_product.phtml')->toHtml());
            Mage::getSingleton('core/session')->setAwproductrecentlyremove(Mage::app()->getLayout()->createBlock('catalog/product_list')->setTemplate('ajaxcartpro/removed_product.phtml')->toHtml());
    ...
    private function _collectActionData()
    {
        ...
        $_freeshipping=Mage::helper("ampromo/bonus")->GetFreeShippingGiftId();
        if($_freeshipping==$actionData['removed_product'])
        $actionData['removed_freeshpping'] =1;
