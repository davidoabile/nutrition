...
class Mage_Catalog_Helper_Product extends Mage_Core_Helper_Url {
    ...
    public function initProduct($productId, $controller, $params = null) {
        ...
        $product = Mage::getModel('catalog/product')
                ...
                ->load($productId);
        if (strpos(Mage::helper('core/http')->getHttpReferer(), 'nutritionwarehouse') === false && (int) $product->getVisibility() === Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE) {
            if ($product->getTypeId() == Mage_Catalog_Model_Product_Type::DEFAULT_TYPE) {
                ...
                if (isset($parentIds[0])) {
                    ...
                    if ($parent->getId()) {
                        Mage::app()->getFrontController()->getResponse()->setRedirect($parent->getProductUrl());
                    }
                }
            }
        }
