...
class Fishpig_AttributeSplash_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	function getUrlSplash($options_id){
		$url = "";
		 $collection = Mage::getResourceModel('attributeSplash/page_collection')->addFieldToFilter("option_id",intval($options_id))
				->addStoreFilter(Mage::app()->getStore())
				->setPageSize(1)
				->setCurPage(1)
				;
		if($collection->getSize()){
			...
			$url = Mage::getUrl() . 'catalogsearch/advanced/result/?manufacturer=' . $options_id;
		}
