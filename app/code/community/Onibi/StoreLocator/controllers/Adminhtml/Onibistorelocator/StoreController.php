<?php
class Onibi_StoreLocator_Adminhtml_Onibistorelocator_StoreController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        if ($postData = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['marker']['name']) and (file_exists($_FILES['marker']['tmp_name']))) {
                try {
                    ...
                    //$postData['marker'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $locMarker .  $_FILES['marker']['name'];
 	...
 	if(isset($_FILES['manager_image']['name']) and (file_exists($_FILES['manager_image']['tmp_name']))) {
                try {
                    $uploader = new Varien_File_Uploader('manager_image');
                    $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
                    $uploader->setAllowRenameFiles(false);
                    $uploader->setFilesDispersion(false);
                    $locMarker = 'storelocator/manager/';
                    $path = Mage::getBaseDir('media') . DS . $locMarker ;
                    $uploader->save($path, $_FILES['manager_image']['name']);
                    $postData['manager_image'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . $locMarker . $_FILES['manager_image']['name'];
                }catch(Exception $e) {
            
                }
            }else{
                if(isset($postData['manager_image']['delete']) && $postData['manager_image']['delete'] == 1){
                    $postData['manager_image'] = '';
                }else{
                    unset($postData['manager_image']);
                }
            }
