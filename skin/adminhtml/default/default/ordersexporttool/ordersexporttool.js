...
var ordersexporttool = {
    ...
    textMode: function() {
        ...
        if ($('file_header').value != "" && $('file_body').value != "")
            ordersexporttool.jsonToTextFields();
    ...
    checkSyntax: function() {
        ...
        if (!ordersexporttool.isXmlMode()) {
            ...
            if ($('file_protector').value != "\"")
                textContent += "enclosure=\"" + $('file_protector').options[$('file_protector').selectedIndex].innerHTML + "\" ";
            else
                textContent += "enclosure='" + $('file_protector').options[$('file_protector').selectedIndex].innerHTML + "' ";
            textContent += ">\n";
    ...
    changeMode: function() {
        ...
        if (ordersexporttool.currentMode == null) {
            ...
            if (ordersexporttool.isXmlMode())
                ordersexporttool.xmlMode();
            else
                ordersexporttool.textMode();
            ...
            if ((ordersexporttool.currentMode > 1 && ordersexporttool.getIdMode() == 1) | (ordersexporttool.currentMode == 1 && ordersexporttool.getIdMode() > 1)) {
                if (confirm("Changing file type from/to xml will clear all your setting.\ Do you want to continue ?")) {
                    ...
                    ordersexporttool.setIdMode(ordersexporttool.currentMode);
                ordersexporttool.currentMode = ordersexporttool.getIdMode();
            }
        }
...
document.observe('dom:loaded', function() {
    ...
    if (!$('file_scheduled_task').value.isJSON())
        $('file_scheduled_task').value = '{"days":[],"hours":[]}';
