...
ol,menu { display: none; margin:1em 0 0 0; padding:0.2em; margin-left:1em;}
...
$config = array(
    'flush_path' => '/apc/clear_server_cache.php',
    'stats_path' => '/apc/stats.php',
    'servers' => array(
        '10.0.1.20',
        '10.0.3.21'
    )
...
foreach ($config['servers'] as $serverName) {
    
    $matches = array();
    
    $flushUrl = "http://{$serverName}{$config['flush_path']}";
    $statsUrl = "http://{$serverName}{$config['stats_path']}";
    
    $flushContent = file_get_contents($flushUrl);
    echo "<h1>" . $serverName . " flush status - " . $flushContent . "</h1>\n";
    
    preg_match("/<body[^>]*>(.*?)<\/body>/is", file_get_contents($statsUrl), $matches);
